...
int init_useful_structure(ckvs_memrecord_t *mr, CKVS_t *ckvs, ckvs_entry_t **out, const char *filename,
 ...
 */
int init_useful_structure(ckvs_memrecord_t *mr, CKVS_t *ckvs, ckvs_entry_t **out,
...
int ckvs_local_getset(const char *filename, const char *key, const char *pwd, const char *set_value)
{
    ...
    const int err_init = init_useful_structure(&mr, &ckvs, &out, filename, key, pwd);
...
int ckvs_local_new(const char *filename, int optargc, char** optargv)
{
    ...
    const int err_init = init_useful_structure(&mr, &ckvs, &out, filename, key, password);
    if (err_init != ERR_NONE)
    {
        ckvs_close(&ckvs);
        return err_init;
...
//=======================================================================================
/**
 * @note Week 14: Extension (very simple extension, without modularisation)
 *
 * @see ckvs_local.h
 */
int ckvs_local_delete(const char *filename, int optargc, char** optargv)
{
    ...
    /* ---------------------------- Find entry and init cryptographic ---------------------------- */
    CKVS_t ckvs;
    ckvs_memrecord_t mr;
    ckvs_entry_t *out = NULL;
    ...
    /* ---------------------------- Delete the entry ---------------------------- */
    /* the entry is not actually deleted from the database, but just considered
     * invalid, so that if you set the entry again then it will be written over */
    out->key[0] = '\0';


    /* ---------------------------- Update the database ---------------------------- */
    const int update_entry = ckvs_write_entry_to_disk(&ckvs, (const uint32_t) (out - ckvs.entries));
    if (update_entry != ERR_NONE)
    {
        ckvs_close(&ckvs);
        return update_entry;
    }
 ...
 * @see ckvs_local.h
 */
int ckvs_local_create(const char *filename, int optargc, _unused char** optargv)
{
    /* ---------------------------- Check ---------------------------- */
    ARG_CHECK(0, optargc);
    M_REQUIRE_NON_NULL(filename);

    return ckvs_new_database(filename);
}
