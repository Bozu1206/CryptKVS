...
struct ckvs_command_mapping
{
    const char* name;
    const char* desc;
    const ckvs_command local;
    const ckvs_command client;
};
...
{
    {.name = "stats", .desc = "", .local = ckvs_local_stats, .client = ckvs_client_stats},
    {.name = "get", .desc = "<key> <password>", .local = ckvs_local_get, .client = ckvs_client_get},
    {.name = "set", .desc = "<key> <password> <filename>", .local = ckvs_local_set, .client = ckvs_client_set},
    {.name = "new", .desc = "<key> <password>", .local = ckvs_local_new, .client = ckvs_client_new},
    {.name = "httpd", .desc = "<database> httpd <url>", .local = ckvs_httpd_mainloop, .client = NULL},
    {.name = "delete", .desc = "<key> <password>", .local = ckvs_local_delete, .client = NULL},
    {.name = "create", .desc = "", .local = ckvs_local_create, .client = NULL}
};
...
int ckvs_do_one_cmd(int argc, char *argv[])
{
    ...
    for (size_t index = 0; index < number_commands; ++index)
    {
        if (strcmp(command[index].name, cmd) == 0)
        {
            ...
            {
                return command[index].client(db_filename, argc - MIN_CMD_SIZE, argv + MIN_CMD_SIZE);
            }

            return command[index].local(db_filename, argc - MIN_CMD_SIZE, argv + MIN_CMD_SIZE);
        }
    }
