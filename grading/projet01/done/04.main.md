...
La gestion des mots de passe est un bon exemple.  Le site [haveibeenpwned](https://haveibeenpwned.com/) regroupe un ensemble de données volées dans plus de 500 sites web, pour un total de 11 milliards de comptes.  Votre email en fait peut-etre partie, du reste.
...
* le serveur génère la valeur `c2` de manière aléatoire (32 octests) ; il stocke dans une table `key`, `auth_key` and `c2` ;
...
* seul le client peut générer `master_key`, qui est ensuite utilisée par le client comme clef symmétrique pour encrypter (écriture) ou décrypter (lecture) la valeur.
