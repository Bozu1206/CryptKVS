...
static int compute_hmac(unsigned char *to_compute, ckvs_memrecord_t *mr, const char *msg)
{
    ...
    M_REQUIRE(to_compute_size == SHA256_DIGEST_LENGTH, ERR_INVALID_COMMAND, "");
    return ERR_NONE;
}
...
int ckvs_client_encrypt_pwd(ckvs_memrecord_t *mr, const char *key, const char *pwd)
{
    ...
    M_REQUIRE(buffer != NULL, ERR_OUT_OF_MEMORY, "");

    strcat(buffer, key);
    strcat(buffer, SEPARATOR);
    strncat(buffer, pwd, strlen(pwd));
    ...
    M_REQUIRE(err_auth == ERR_NONE, err_auth, "");

    const int err_c1 = compute_hmac(mr->c1.sha, mr, C1_MESSAGE);
    M_REQUIRE(err_c1 == ERR_NONE, err_c1, "");
