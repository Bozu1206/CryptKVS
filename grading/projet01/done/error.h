...
// ----------------------------------------------------------------------
/**
 * @brief M_REQUIRE_NON_NULL_VARGS macro is useful for requiring a list of non-NULL arguments
 *        Example usage:
 *            int my_favorite_function(struct whatever* key, const char *filename)
 *                  {
 *                      M_REQUIRE_NON_NULL_VARGS(key, filename)
 *
 */
#define M_REQUIRE_NON_NULL_VARGS(...)                  \
    do {                                               \
        unsigned int i;                                \
        const void *p[] = { __VA_ARGS__ };             \
        for (i = 0; i < sizeof(p)/sizeof(*p); ++i)     \
        {                                              \
            M_REQUIRE_NON_NULL(p[i]);                  \
        }                                              \
    } while(0)                                         \

// ----------------------------------------------------------------------
/**
 * @brief M_REQUIRE_NON_NULL_VOID_FUNCTION macro is useful for requiring non-NULL argument in
 *        function that returns void.
 *        Example usage:
 *          void function(type *arg)
 *          {
 *              M_REQUIRE_NON_NULL_FUNCTION(arg);
 */
#define M_REQUIRE_NON_NULL_VOID_FUNCTION(arg)     \
    do {                                          \
        if ((arg) == NULL)                        \
        {                                         \
            return;                               \
...
// ----------------------------------------------------------------------
/**
 * @brief M_FREE_MEMORY macro is useful to avoid code duplication and to clean memory on error
 *        Also close the database using ckvs_close (@see ckvs_io.c)
 *
 *        Example usage:
 *            int my_favorite_function(struct whatever* key, const char *filename)
 *                  {
 *                      CKVS_t ckvs;
 *                      type * ptr1 = calloc(1, sizeof(type))
 *                      type * ptr2 = calloc(1, sizeof(type))
 *                      if (error)
 *                          M_FREE_MEMORY(ckvs, ptr1, ptr2);
 *
 */
#define M_FREE_MEMORY(CKVS, ...)                      \
    do {                                              \
        unsigned int i;                               \
        void *p[] = { __VA_ARGS__ };                  \
        for (i = 0; i < sizeof(p)/sizeof(*p); ++i)    \
