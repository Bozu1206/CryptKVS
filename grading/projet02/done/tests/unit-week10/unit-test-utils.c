...
// ======================================================================
START_TEST(sha_from_string_NULL) {
// ------------------------------------------------------------
#ifdef WITH_PRINT
    printf("=== %s:\n", __func__);
#endif
    ...
    ck_assert_int_eq(SHA256_from_string(NULL, NULL), ERR_INVALID_ARGUMENT);
    ck_assert_int_eq(SHA256_from_string(in, NULL), ERR_INVALID_ARGUMENT);
    ck_assert_int_eq(SHA256_from_string(NULL, &sha), ERR_INVALID_ARGUMENT);
...
// ------------------------------------------------------------
    ...
    ckvs_sha_t sha;
    const char *message = "thekey|1";
    SHA256((const unsigned char *) message, strlen(message), sha.sha);

    ckvs_sha_t sha_out;
    const char *hex_message = "4fd465936234243a301a9b87301b0a6545c68d1093c6acfe26d87959a2d1355d";
    SHA256_from_string(hex_message, &sha_out);

    ck_assert_int_eq(ckvs_cmp_sha(&sha, &sha_out), 0);
#ifdef WITH_PRINT
    printf("=== END of %s\n", __func__);
#endif
}
...
// ------------------------------------------------------------
    ...
    ckvs_sha_t sha;
    const char *message = "voici un message plutot long contenant des espaces";
    SHA256((const unsigned char *) message, strlen(message), sha.sha);

    ckvs_sha_t sha_out;
    const char *hex_message = "6d6882c0a10c7e83d6f4b1a3a533b81fc9ec5fb292cb38325c3e85c406e65a93";
    SHA256_from_string(hex_message, &sha_out);
...
// ------------------------------------------------------------
    ...
    ckvs_sha_t sha_out;
    const char *hex_message = "5488613c42b0d34d60f7aa9e94be317a3ee102a2bbd91ccc73cc79fbc2269955";
    SHA256_from_string(hex_message, &sha_out);
...
#pragma GCC diagnostic pop
    ...
    tcase_add_test(tc1, sha_from_string_NULL);
    tcase_add_test(tc1, sha_from_string_1);
    tcase_add_test(tc1, sha_from_string_2);
    tcase_add_test(tc1, sha_from_string_3);
    return s;
}
